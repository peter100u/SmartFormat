...
- 用户轻量偏好首版与任务记录统一使用 Hive box 管理，不再单独引入 `shared_preferences`。
...
- Drift 测试文档：<https://drift.simonbinder.eu/testing/>
...
- Drift 测试优先使用内存数据库，数据库实例在 `setUp` 中创建、在 `tearDown` 中关闭；Widget 测试里要注意流关闭时序，避免测试结束后仍有挂起 stream/timer。
- Drift 涉及流更新时，优先断言真实的发射顺序，例如先插入、再更新、再校验 stream 的顺序输出，而不是只看最终结果。
...
- repository 测试先参考 Drift 官方测试结构，优先测真实数据库行为，不要只 mock repository 自己。
