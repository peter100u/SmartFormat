...
2. 配置 Material 3、Riverpod、go_router、Hive CE、代码生成和 lint。
...
- 任何用户可感知的耗时操作，都必须给出明确等待反馈。
- 不允许点击后长时间无响应、无 loading、无禁用态、无进度提示。
- 至少以下场景必须展示等待态：文件选择后的预检、任务创建、缓存清理、反馈发送、页面内 WebView 首次加载、许可证内容加载、媒体探测、FFmpeg 任务启动前准备。
- 等待时间很短也可以显示轻量 loading，只要用户能明确知道系统正在处理中。
...
- `repository` 负责数据读写，例如 Hive box、配置、历史记录和本地缓存。
...
- 不默认编写 DTO 和 mapper，除非持久化结构、平台返回结构和业务模型确实需要分开。
...
- UI 不直接调用 repository、service、Hive、FFmpeg 或平台插件。
...
项目代码注释统一使用中文。除第三方生成代码、外部依赖源码或必须保留原文的协议文本外，手写业务代码中的文档注释、行内注释和 `TODO` 注释都应使用中文，保证团队约定、技术文档和实际代码风格一致。
...
///
/// 边界：
/// - UI 通过这个 controller 转发用户动作。
/// - 这个 controller 负责更新 `TaskState`。
/// - 数据读写通过 `TaskRepository` 完成。
/// - 它不应直接调用平台插件。
```
...
Mova 当前使用 Hive CE 作为本地持久化方案。

Hive CE 适合 Mova 当前实现的原因：

- 任务记录和轻量偏好都可以直接落在本地 box 中，结构简单，适合 MVP 离线优先需求。
- 初始化和跨平台接入成本低，已与当前 Flutter 工程集成。
- 任务历史、语言偏好等数据量较小，不需要提前引入更重的数据库抽象。
- 后续如果筛选、关联查询或迁移需求明显变复杂，再评估是否升级到更重的持久化方案。

MVP 阶段持久化重点服务于任务记录和用户偏好，不应过早承担复杂业务逻辑。
...
- 当前产品规划以 `ffmpeg_kit_extended_flutter` 为基础，当前目标配置为 `type: "base"`, `gpl: false`, `small: true`；上线前仍必须确认实际构建类型、GPL 状态、启用组件和许可证披露。
...
- `flutter_inappwebview`：用于应用内展示隐私政策、用户协议、许可证披露页或其他需要嵌入展示的网页内容。
...
Mova MVP 设置页包含多语言和首次启动说明，后续还可能加入主题模式、默认输出位置等偏好，因此需要一套既能承载任务记录，也能承载轻量偏好的本地持久化方案。

- `flutter_localizations`：使用 Flutter SDK 自带本地化支持。
- `intl`：用于日期、数字、文件大小、任务时间和多语言文案格式化。
- `hive_ce`：用于语言、首次启动说明是否展示等轻量偏好，以及任务记录这类本地结构化数据。主题模式、默认质量预设和默认输出位置后续再加入。

当前实现统一使用 Hive 管理任务记录与少量 key-value 偏好。不要为一个简单偏好再额外引入第二套持久化方案；独立最近文件列表、用户自定义工具预设后续再评估。
...
- 用户协议
- 许可证披露页
- 后续可能加入的帮助中心、活动说明或公告页

这里统一选用 `flutter_inappwebview: 6.2.0-beta.3` 作为 WebView 能力基础，参考：

- <https://pub.dev/packages/flutter_inappwebview/versions/6.2.0-beta.3>

选用原因：

- 能力边界更完整，后续如果需要处理加载进度、导航拦截、错误页、Cookie、下载拦截或 JavaScript 通信，不需要很快换库。
- 同时支持 inline webview、headless webview 和 in-app browser，便于未来从简单法律页扩展到更复杂场景。
- 对设置页、帮助页和运营内容页的承载上限更高。

使用硬规则：

- 业务页面禁止直接使用 `InAppWebView`。
- 业务页面禁止直接 import `flutter_inappwebview`。
- 必须在 `shared/widgets/` 或 `shared/webview/` 下封装 Mova 自己的 WebView 组件和适配层。
- 页面层只依赖 Mova 的封装，例如 `MovaWebViewPage`、`MovaEmbeddedWebView`、`MovaWebViewConfig`，不直接依赖第三方插件 API。

推荐封装：

```dart
class MovaWebViewPage extends StatelessWidget {
  final String title;
...
```dart
class MovaEmbeddedWebView extends StatelessWidget {
  final Uri initialUri;
...
- 外链跳转策略，例如交给外部浏览器。
- 返回键、前进后退和页面刷新能力。
- 是否允许 JavaScript、文件访问、多窗口、下载等安全相关开关。
- 通用埋点入口，例如页面打开、加载成功、加载失败。
- 法律文档页和普通网页的默认配置差异。

安全规则：

- 默认只加载受信任域名或产品明确配置的 URL。
- 默认关闭非必需的 JavaScript bridge、文件访问和高风险能力，只有明确业务需要时才由封装层白名单开启。
- 域名放行、Cookie 策略、证书异常处理、下载拦截和 JS 通信都必须集中写在封装层，不能散落到业务页面。
- 法律文档页优先使用静态内容或受控地址，页面层不允许随意拼接未知 URL。

### 等待态与弹窗反馈

Mova 需要统一处理耗时操作的等待态、全局 loading、轻提示和业务弹窗反馈。这里选用：
...
- 展示轻提示
- 在异步业务处理中提供统一等待反馈
- 让跨页面、跨路由的耗时操作仍能维持一致的交互体验

使用原则：

- 耗时业务默认显示 loading，不允许只在日志里处理而不给用户反馈。
- loading 的出现与关闭必须和真实异步生命周期绑定，不能靠固定延时关闭。
- 成功后关闭 loading；失败后关闭 loading，并给出用户可理解的错误反馈。
- 页面局部小范围加载优先用页面内 skeleton、占位或局部进度；跨页面或全局异步流程优先用 `flutter_smart_dialog`。
- 一人开发阶段允许直接使用 `SmartDialog.showLoading()`、`SmartDialog.dismiss()`、`SmartDialog.showToast()` 等官方 API，不强制额外封装一层。
- 但同一功能内要保持写法一致，不要一部分用 `SmartDialog`，另一部分又切回零散的 `showDialog`、`ScaffoldMessenger` 或自写 overlay。
- 如果后续多人协作、交互规则变复杂、或 loading/toast 用法开始出现重复分叉，再考虑补项目级封装。

接入要求：

- `MaterialApp.router` 外层统一接入 `FlutterSmartDialog.init()`。
- 路由切换、异步取消、页面销毁和异常抛出时，要确保 loading 不残留。
- 同一业务链路避免重复弹多个 loading；直接使用时也要自己控制好显示和关闭的对应关系。
...
- 当现有持久化模型已经足够表达存储结果时，不重复创建一套几乎相同的 Freezed 模型。
...
- `hive_ce` 和 `hive_ce_flutter`：统一保存任务记录和轻量本地偏好，并负责 Flutter 场景下的本地初始化。
- `path_provider`：用于缓存目录、临时目录和结果文件目录等路径获取。
...
- 额外底层数据库绑定：当前不直接加入。现有实现使用 Hive CE 持久化任务记录和轻量偏好；只有后续明确切换到 SQLite 方案时，再评估是否引入。
...
不要手动修改 `.g.dart`、`.freezed.dart` 和 Hive 生成文件。
...
- `data/repositories/*`：组合 Hive 持久化和轻量数据转换，不执行平台插件。
- `services/*`：封装平台插件、副作用和 FFmpeg。
- `shared/tool_registry/*`：静态注册工具，不保存运行时状态。
- `shared/widgets/*`：跨功能 UI 组件，不直接创建任务。
- `shared/webview/*` 或 `shared/widgets/mova_webview*.dart`：统一封装 WebView 组件、配置和插件适配层，业务模块不得直接依赖 `flutter_inappwebview`。
...
- Hive 持久化模型不强制再包一层 Freezed。
- 如果持久化字段和 UI 字段差异明显，再写 mapper。

## Hive 持久化实施

相关文件：

- `shared/models/task_models.dart`
- `shared/models/task_hive_adapters.dart`
- `data/repositories/task_repository.dart`
- 用户轻量偏好首版与任务记录统一使用 Hive box 管理，不再单独引入额外偏好存储依赖。
...
- 先参考所依赖类库自己的官方测试文档、官方示例和仓库内测试用例，再设计 Mova 的测试。
- 不要先凭主观想象写测试夹具；优先复用类库作者已经验证过的初始化、销毁、fake、内存实现、回调断言和异步等待模式。
- 测试要尽量贴近依赖库真实使用边界，验证“输入 -> 调用 -> 状态变化/返回值/回调/副作用”的完整链路，而不是只测表面字段。
- 遇到数据库、FFmpeg、文件系统、回调桥接、平台通道等有时序和资源释放要求的模块时，先看上游测试怎么建环境、怎么清理，再决定我们自己的测试结构。

参考规则：

- Hive、Riverpod、go_router、FFmpegKit、url_launcher 等依赖，只要项目里实际使用到，对应模块写测试前都应该先阅读官方文档或仓库测试。
- 优先级顺序：
  1. 官方测试文档
  2. 官方仓库测试文件
  3. 官方最小示例
  4. 我们项目自己的既有测试模式
- 如果上游已经给出推荐测试姿势，Mova 默认跟随，不自行发明另一套风格，除非有明确理由。

当前必须参考的基准：

- FFmpegKit Extended Flutter 测试用例：<https://github.com/akashskypatel/ffmpeg-kit-extended/blob/master/flutter/test/api_test.dart>

从这些基准里应该吸收的具体方法：

- Hive 相关测试优先使用临时目录和隔离 box，在 `setUp` 中初始化、在 `tearDown` 中关闭；Widget 测试里要注意流关闭时序，避免测试结束后仍有挂起 stream/timer。
- Hive 涉及流更新时，优先断言真实的发射顺序，例如先插入、再更新、再校验 stream 的顺序输出，而不是只看最终结果。
- 涉及数据库时间、当前时间、默认时间字段时，要使用可确定的时间策略，避免测试依赖真实系统时间。
- FFmpeg/原生桥接测试要优先验证初始化、清理、会话重置、回调注册/注销、异常路径和资源释放。
- 带原生回调的测试要在每个用例前后清空全局状态，避免“上一个测试残留监听器影响下一个测试”。
- 文件、缓存和临时目录相关测试要先构造隔离目录，在 `tearDown` 或 `tearDownAll` 中清理，不能污染开发机真实环境。
...
- repository 测试优先测真实 Hive box 行为，不要只 mock repository 自己。
- service 测试要按依赖库边界写，例如 `url_launcher` 测“是否按预期构造并发起调用”，FFmpeg 测“命令、会话、回调、状态流是否正确”。
- controller 测试要验证状态变化和副作用委托，不要只断言方法被调用一次。
- Widget 测试要验证页面在真实 provider 覆盖下的交互闭环，包括导航、提示、禁用态、异步加载态和错误反馈。
- 新接入一个第三方库时，应同步补一条技术说明：这个库测试时优先参考哪份官方文档或哪几个官方测试文件。
...
3. 创建核心模型和工具注册表。
4. 创建 Hive 持久化模型、box 初始化和 `TaskRepository`。
...
- 核心文件必须用简短中文注释说明职责、边界、状态流和副作用，方便长期维护和 AI 辅助理解。
...
- 不要让 Widget 直接依赖 Hive、FFmpeg、文件选择器、相册插件或权限插件。
