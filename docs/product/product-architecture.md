...
产品目标不是打败所有专业媒体工具，而是覆盖足够常见的格式转换、压缩、提取、多文件输入和任务记录场景，成为一款可以支撑一人公司持续经营的日常工具。
...
音频降噪有价值，但质量敏感，不进入 MVP。第一版先保证格式转换、压缩和提取音频的稳定性。
...
每条任务记录都应该展示输入文件、输出格式、处理进度、输出路径，以及失败时清晰的错误说明。
